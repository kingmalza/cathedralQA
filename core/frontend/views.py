...
def lic_register(request, reg_status=None, **kwargs):
    ...
    if reg_status:
        ...
        if reg_status == 'OK':
            con_stat = "<div id='overlay_demo' style='display:block'><div id='text-demo'><div class='login-box-body'><p class='login-box-msg'><strong><font color='green'>YOU ARE IN A DEMO AREA</font></strong></p><br>The Aida environment you have access to is used only for demonstration purposes.<br>Data contained within are seen and can be managed by several users. Use this environment just for familiarize with the test suite and its characteristics.<br><br>To use a real dedicated Aida environment, create an account through the '<strong>Create account</strong>' buttons.<br><br><div><form><a href='/register/retry'><button class='btn btn-block btn-success btn-lg'>RETRY THE REGISTRATION PROCES</button></a></form></div></div></div></div>"
        elif reg_status == 'KO':
