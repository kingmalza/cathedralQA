...
from frontend.models import temp_main, temp_case, temp_variables, temp_library, temp_test_keywords, temp_keywords, temp_pers_keywords, import_his
...
def import_templ(request):
    ...
    if request.POST:
        ...
        ck_stripe = get_lic()
        print("Tproceee-> ", request.POST['tprice'])


        if (ck_stripe['LDATA'][9]):

            #If customer is registered check if there are enought credit for buying selected template
            t_price = request.POST['tprice']
            mycredits = ck_stripe['LDATA'][8]
            if int(mycredits)-int(t_price) >= 0:
